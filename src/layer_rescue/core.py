...
def _resume_preamble(
    ...
    lines = [
        ...
        "; Generated by Layer Rescue 0.1.0",
        f"; Resume at layer {layer.number}/{layer.total}, Z={_format_number(layer.z)} mm",
        "; SAFETY: requires retained Z coordinates and a firmly attached original part.",
        f"M73 P{progress}",
    ]
    ...
    lines.extend(
        [
            "G92 E0",
            f"G1 Z{_format_number(layer.z)} F600 ; approach the recovered layer at the rear station",
            "M1002 gcode_claim_action : 0",
            "; LAYER_RESCUE_BLOCK_END",
        ]
    )
    return lines, nozzle, bed


def _validate_output(text: str, start_layer: int, total_layers: int) -> None:
    layers = [
        int(match.group(1))
...
def build_resume_gcode(text: str, options: ResumeOptions) -> tuple[str, ResumeReport]:
    ...
    preamble, nozzle, bed = _resume_preamble(analysis, layer, state, options)
    ...
    _validate_output(output, options.start_layer, analysis.total_layers)
